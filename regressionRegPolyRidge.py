...
from sklearn.model_selection import cross_validate, KFold, GridSearchCV, learning_curve
...
def regression_regularized_polyfeat_ridge(X_train, y_train):
    ...
    fig.text(0.04, 0.5, 'Mean squared error score to minimize (-)', va='center', rotation='vertical')
    ...
    results = learning_curve(
        grid_search_0, X_train_0, y_train_0, train_sizes=train_sizes, cv=kf_indices_0,
    ...
    plt.title("Regressor 0 Ridge learning curve : assessing influence of training set size")
    plt.show()
    ...
    # Model or grid_search.best_estimator_
    results = learning_curve(
        grid_search_1, X_train_1, y_train_1, train_sizes=train_sizes, cv=kf_indices_1,
        scoring='neg_mean_squared_error', n_jobs=2)
    train_size, train_scores, test_scores = results[:3]
    # Correction for 'neg_mean_squared_error'
    train_errors, test_errors = -train_scores, -test_scores
    ...
    plt.xlabel("Number of samples in the training set")
    plt.ylabel("Mean squared error (-)")
    plt.title("Regressor 1 Ridge learning curve : assessing influence of training set size")
    plt.show()
    ...
    # Model or grid_search.best_estimator_
    results = learning_curve(
        grid_search_2, X_train_2, y_train_2, train_sizes=train_sizes, cv=kf_indices_2,
        scoring='neg_mean_squared_error', n_jobs=2)
    train_size, train_scores, test_scores = results[:3]
    # Correction for 'neg_mean_squared_error'
    train_errors, test_errors = -train_scores, -test_scores
    ...
    plt.xlabel("Number of samples in the training set")
    plt.ylabel("Mean squared error (-)")
    plt.title("Regressor 2 Ridge learning curve : assessing influence of training set size")
    plt.show()
