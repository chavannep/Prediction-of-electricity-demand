...
from multiClassBagging import multiclass_bagging
from multiClassGBDT import multiclass_gbdt
from multiClassRandomForest import multiclass_random_forest
...
model_bagging = multiclass_bagging(X_train, X_test, y_train, y_test, kf_indices)
model_random_forest = multiclass_random_forest(X_train, X_test, y_train, y_test, kf_indices)
model_svm = multiclass_svm(X_train, X_test, y_train, y_test, kf_indices)
model_gbdt = multiclass_gbdt(X_train, X_test, y_train, y_test, kf_indices)
multiclass_dict['KNN'] = (model_knn.best_estimator_, model_knn.best_score_)
multiclass_dict['Bagging'] = (model_bagging.best_estimator_, model_bagging.best_score_)
multiclass_dict['Random_forest'] = (model_random_forest.best_estimator_, model_random_forest.best_score_)
multiclass_dict['SVM'] = (model_svm.best_estimator_, model_svm.best_score_)
multiclass_dict['GBDT'] = (model_gbdt.best_estimator_, model_gbdt.best_score_)
...
df_tmp = df[['location', 'elec_demand_cluster', 'elec_prediction']]
df = pd.concat([df_tmp, df_tmp_1], join = 'outer', axis = 1)
...
print('\nTotal electricity supply forecast (kwh) : ', total_elec_demand)
